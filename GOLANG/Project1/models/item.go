...
func GetAllItems(conn *pgx.Conn) ([]Item, error) {
	rows, err := conn.Query(context.Background(), "SELECT id, title, notes, seller_id, price FROM item")
	if err != nil {
		fmt.Println(err)
	...
	for rows.Next() {
		item := Item{}
		err = rows.Scan(&item.ID, &item.Title, &item.Notes, &item.SellerID, &item.Price)
		if err != nil {
			fmt.Println(err)
			continue
		}
